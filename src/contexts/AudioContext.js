...
const SHOW_NOTIFICATIONS_KEY = "once_show_notifications";
const PREVIOUS_MUTE_STATE_KEY = "once_previous_mute_state";
...
export const AudioProvider = ({ children }) => {
  ...
  const [muted, setMutedState] = useState(() => {
    ...
    console.log(
      `초기 음소거 로드 (로컬 스토리지): ${initialMuted}, 최종 결정: ${finalMuted} (볼륨 ${initialVolume})`
    );
    return finalMuted;
  });

  // 알림 표시 여부 상태 추가
  const [showNotifications, setShowNotificationsState] = useState(() => {
    const initialShowNotifications = loadFromLocalStorage(
      SHOW_NOTIFICATIONS_KEY,
      true
    );
    console.log(
      "초기 알림 표시 여부 로드 (로컬 스토리지):",
      initialShowNotifications
    );
    return initialShowNotifications;
  });

  // 음소거 상태를 저장하는 ref 추가
  const previousMutedStateRef = useRef(
    loadFromLocalStorage(PREVIOUS_MUTE_STATE_KEY, muted)
  );
      ...
      if (window.electron && window.electron.getAudioSettings) {
        try {
          ...
          if (!hasVolumeInStorage) {
            console.log(
              "로컬 스토리지에 볼륨 설정이 없어 Electron 설정 적용:",
              settings.volume
            );
          ...
          if (!hasMutedInStorage) {
            ...
            console.log(
              "로컬 스토리지에 음소거 설정이 없어 Electron 설정 적용:",
              electronMuted
            );
  ...
  const setVolume = useCallback(
    ...
    [muted, audioInstance]
  ); // toggleMute 의존성 제거 (아래 toggleMute에서 처리)
  ...
  const toggleMute = useCallback(
    ...
    [muted, volume, audioInstance]
  ); // volume 의존성 추가

  // 알림 표시 여부 토글 핸들러
  const toggleShowNotifications = useCallback(
    (forceShow) => {
      // forceShow가 boolean이면 그 값을 사용, 아니면 현재 상태 토글
      const newShowNotifications =
        typeof forceShow === "boolean" ? forceShow : !showNotifications;
      console.log(
        `알림 표시 토글 요청: ${forceShow} -> ${newShowNotifications}`
      );

      // 알림을 끄는 경우 (true -> false)
      if (showNotifications && !newShowNotifications) {
        // 현재 음소거 상태를 저장
        previousMutedStateRef.current = muted;
        // 로컬 스토리지에도 저장
        try {
          localStorage.setItem(PREVIOUS_MUTE_STATE_KEY, muted.toString());
          console.log("이전 음소거 상태 저장:", muted);
        } catch (e) {
          console.error("이전 음소거 상태 저장 실패:", e);
        }
      ...
      // 알림을 켜는 경우 (false -> true)
      else if (!showNotifications && newShowNotifications) {
        // 이전 음소거 상태 복원 (볼륨이 0인 경우는 무조건 음소거 유지)
        if (volume > 0) {
          const previousMuted = previousMutedStateRef.current;
          console.log("이전 음소거 상태 복원:", previousMuted);
          toggleMute(previousMuted);
        }
      }

      // 상태 업데이트
      setShowNotificationsState(newShowNotifications);

      // 로컬 스토리지 저장
      try {
        localStorage.setItem(
          SHOW_NOTIFICATIONS_KEY,
          newShowNotifications.toString()
        );
        console.log(
          "알림 표시 상태 로컬 스토리지에 저장:",
          newShowNotifications
        );
      } catch (e) {
        console.error("알림 표시 상태 로컬 스토리지 저장 실패:", e);
      }

      // Electron에 변경 알림 (필요한 경우)
      if (window.electron && window.electron.send) {
        window.electron.send("set-notifications-enabled", newShowNotifications);
      }
    },
    [showNotifications, muted, toggleMute, volume]
  );
  ...
  useEffect(() => {
    const handleVolumeChange = (event, newVolume) => {
      ...
      if (typeof newVolume === "undefined") {
        console.error(
          "Electron 외부 볼륨 변경 감지 오류: 수신된 볼륨 값이 undefined입니다."
        );
        return; // 처리 중단
      }
    ...
    const handleMuteChange = (event, newMuted) => {
      ...
      if (typeof newMuted === "undefined") {
        console.error(
          "Electron 외부 음소거 변경 감지 오류: 수신된 음소거 값이 undefined입니다."
        );
        return; // 처리 중단
      }
  ...
  const playNotificationSound = useCallback(
    (soundFile) => {
      try {
        ...
        // 알림이 비활성화되어 있으면 재생하지 않음
        if (!showNotifications) {
          console.log("알림 비활성화 상태: 알림음 재생 건너뜀");
          return;
        }
    ...
    [volume, muted, showNotifications]
  );
  ...
  const value = {
    ...
    toggleShowNotifications, // 새로운 핸들러 함수
