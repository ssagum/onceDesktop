...
const DepartmentRoleList = ({
  ...
  Object.entries(roleOptions).forEach(([departmentKey, roles]) => {
    ...
    roles.forEach((role) => {
      if (
        role !== "팀원" &&
        !allPossibleOptions.find((opt) => opt.value === role)
      ) {
        const isLeaderRole =
          role.includes("팀장") ||
          role.includes("과장") ||
  ...
  console.log(
    "DepartmentRoleList - All Possible Options (Revised Team Name Logic):",
    allPossibleOptions
  );
  ...
  if (allowedOptions && Array.isArray(allowedOptions)) {
    ...
    filteredItems = allPossibleOptions.filter((item) =>
      allowedOptions.includes(item.value)
    );
  }

  console.log(
    "DepartmentRoleList - Filtered by Allowed Options:",
    filteredItems
  );
  ...
  if (onlyLeaders) {
    ...
    console.log(
      "DepartmentRoleList - Filtered by onlyLeaders (Writer):",
      finalFilteredItems
    );
  } else {
    ...
    console.log(
      "DepartmentRoleList (Assignee) - Leaders found in allowed list:",
      leadersInList.map((l) => l.value)
    );
    ...
    Object.entries(roleOptions).forEach(([deptKey, roles]) => {
      ...
      roles.forEach((role) => {
        if (role !== "팀원" && teamNameToMap) {
          // 리더 역할이고 해당하는 팀 이름이 있을 때만 매핑
    ...
    // --- --- ---
    console.log(
      "DepartmentRoleList (Assignee) - Leader to Team Map (Revised):",
      leaderToTeamMap
    );
    ...
    leadersInList.forEach((leaderItem) => {
      ...
      if (correspondingTeam) {
        console.log(
          `DepartmentRoleList (Assignee) - Adding team '${correspondingTeam}' for leader '${leaderItem.value}'`
        );
        // 해당 팀이 allPossibleOptions에 실제로 존재하는지 확인 후 추가
        const teamExists = allPossibleOptions.find(
          (opt) => opt.value === correspondingTeam
        );
        if (
          teamExists &&
          !finalFilteredItems.some((item) => item.value === correspondingTeam)
        ) {
    ...
    console.log(
      "DepartmentRoleList (Assignee) - Final list after adding teams:",
      finalFilteredItems.map((i) => i.value)
    );
    // --- --- ---
  }
  ...
  return (
    <div className="w-full max-h-[400px] overflow-y-auto p-4">
      {sortedGroups.length === 0 && (
        <div className="mb-4">
          <div className="font-bold text-gray-700 mb-2">기본 옵션</div>
          <div className="pl-4">
            <div
              className={`py-2 px-4 cursor-pointer rounded-md transition-all duration-200 hover:bg-gray-100`}
              onClick={() => onSelect("대표원장")}
            >
              <span className="font-medium">대표원장</span>
            </div>
          </div>
          <div className="text-xs text-gray-500 mt-2 italic">
            (허용된 옵션이 없어 기본 옵션이 표시됩니다)
          </div>
        </div>
      )}
...
export const getAllowedTaskSelections = (
  ...
  if (fieldType === "writer") {
    ...
    console.log(
      `getAllowedTaskSelections (assignee) - Step A: Initial allowed roles/teams:`,
      [...finalAllowedSet]
    );

    // --- 추가 로직: 허용된 리더의 팀도 추가 ---
    const currentLeaders = [...finalAllowedSet].filter((opt) =>
      allLeaders.includes(opt)
    );
    console.log(
      `getAllowedTaskSelections (assignee) - Step B: Leaders found in allowed set:`,
      currentLeaders
    );
    ...
    currentLeaders.forEach((leaderRole) => {
      ...
      if (correspondingTeam) {
        console.log(
          `getAllowedTaskSelections (assignee) - Step C: For leader ${leaderRole}, found team ${correspondingTeam}. Adding to set.`
        );
        finalAllowedSet.add(correspondingTeam);
      } else {
        console.log(
          `getAllowedTaskSelections (assignee) - Step C: For leader ${leaderRole}, no corresponding team found.`
        );
      }
    });
    // --- ---
    console.log(
      `getAllowedTaskSelections (assignee) - Step D: Allowed options after adding leader's teams:`,
      [...finalAllowedSet]
    );
  }
  ...
  console.log(
    "getAllowedTaskSelections - All Valid Options for Final Filtering:",
    allValidOptionsForFiltering
  );
  // --- --- ---

  // 생성된 유효 옵션 목록(allValidOptionsForFiltering)을 기준으로 최종 필터링
  const finalAllowed = allowed.filter((opt) =>
