  ...
  return (
    <div
      className={`flex flex-col p-2 rounded-lg ${
        isOver ? "bg-blue-100 ring-2 ring-blue-400 shadow-lg" : ""
    ...
    >
      ...
      <div
        ref={setNodeRef}
        className={`flex-1 overflow-y-auto p-1 transition-colors min-h-[250px] max-h-[600px] ${
          isOver ? "bg-blue-50" : ""
        }`}
      >
        <SortableContext
...
const RequestStatusModal = ({
  ...
  const handleDragEnd = (event) => {
    ...
    // 드롭 대상이 컨테이너인지 확인
    if (!over.data.current || over.data.current.type !== "container") {
